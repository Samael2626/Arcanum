# Términos de Servicio — ARCANUM

**Última actualización:** 25 de agosto de 2026

Bienvenido a ARCANUM. Estos Términos de Servicio ("Términos") rigen tu uso de la aplicación móvil ARCANUM y todos sus servicios relacionados. Al usar la App, aceptas estos Términos en su totalidad.

---

## 1. Descripción del servicio

ARCANUM es una aplicación móvil de práctica mágica que ofrece:

- **Carta natal y cálculos astrológicos** — generados a partir de tus datos de nacimiento
- **Oracle IA** — interpretaciones esotéricas basadas en inteligencia artificial
- **Tarot** — tiradas e interpretaciones de los 78 arcanos
- **Grimorio personal** — espacio cifrado para tus notas y práctica
- **Calendario astral** — efemerides y aspectos planetarios
- **Saber** — biblioteca de fuentes históricas y contenido educativo
- **Materia Arcana** — correspondencias de plantas, piedras, metales y perfumes

La App opera bajo un modelo freemium con funcionalidades gratuitas y premium.

---

## 2. Cuenta de usuario

### 2.1 Registro
- Debes tener al menos 18 años para crear una cuenta
- Proporciona información precisa y completa durante el registro
- Eres responsable de mantener la confidencialidad de tu contraseña
- Una cuenta por persona; no se permiten cuentas duplicadas

### 2.2 Seguridad
- Notifica inmediatamente cualquier uso no autorizado de tu cuenta
- No compartas tus credenciales con terceros
- ARCANUM no es responsable por pérdida derivada de uso no autorizado

---

## 3. Suscripciones y pagos

### 3.1 Tier gratuito
- Funcionalidades básicas sin costo
- Límites diarios de uso (tarot: 1/día, oráculo: 1/día, etc.)
- Acceso a anuncios recompensados para obtener créditos adicionales

### 3.2 Tier Premium (Místico)
- **Mensual:** $7.99 USD/mes
- **Anual:** $59.99 USD/año
- Funcionalidades ilimitadas, sin anuncios
- Trial gratuito de 7 días (cancela antes de que termine para no ser cobrado)

### 3.3 Consumibles
- Paquetes de créditos para funciones específicas
- **10 créditos:** $1.99 USD
- **50 créditos:** $7.99 USD
- **Paquete bundle:** $4.99 USD

### 3.4 Procesamiento de pagos
- Los pagos se procesan a través de Google Play Store
- Las suscripciones se gestionan mediante RevenueCat
- Las renovaciones son automáticas salvo cancelación
- Los precios pueden variar según la región

### 3.5 Cancelación
- Cancela en cualquier momento desde Google Play Store
- El acceso premium continúa hasta el final del período facturado
- No se realizan reembolsos parciales por períodos no utilizados

---

## 4. Uso aceptable

### 4.1 Puedes
- Usar la App para fines personales y no comerciales
- Compartir capturas de pantalla de tu contenido generado
- Exportar datos de tu grimorio

### 4.2 No puedes
- Usar la App para fines ilegales o no autorizados
- Reproducir, duplicar o copiar el contenido de la App para redistribución
- Intentar evadir límites de uso o controles de acceso
- Usar bots, scripts o automatizaciones para interactuar con la App
- Realizar ingeniería inversa del código fuente
- Transferir tu cuenta a terceros

---

## 5. Propiedad intelectual

- ARCANUM, su logo, diseño y contenido original son propiedad del desarrollador
- El contenido generado por la App (interpretaciones, cálculos) es tuyo para uso personal
- Las tradiciones esotéricas, sistemas astrológicos y arquetipos son conocimiento público
- No adquirimos derechos sobre el contenido que creas en el grimorio

---

## 6. Contenido esotérico — Descargo de responsabilidad

### 6.1 No es consejo profesional
ARCANUM proporciona contenido esotérico, astrológico y de adivinación únicamente con fines de entretenimiento, autoconocimiento y práctica espiritual. **NO sustituye:**

- Consejo médico, psicológico o de salud
- Asesoría financiera o de inversiones
- Asesoría legal
- Decisiones de vida importantes

### 6.2 Oráculo IA
- Las respuestas del oráculo son generadas por inteligencia artificial
- No representan opiniones profesionales calificadas
- Pueden contener imprecisiones o interpretaciones subjetivas
- Úsalas como guía, no como única fuente de decisión

### 6.3 Carta natal y astrología
- Los cálculos se basan en efemérides estándar
- Las interpretaciones son orientativas
- La astrología no es una ciencia comprobada

---

## 7. Limitación de responsabilidad

En la máxima medida permitida por la ley:

- ARCANUM se proporciona "tal cual" sin garantías de ningún tipo
- No garantizamos disponibilidad ininterrumpida del servicio
- No somos responsables por decisiones tomadas basadas en el contenido de la App
- La responsabilidad total no excederá el monto pagado por suscripción en los últimos 12 meses

---

## 8. Indemnización

Acceptas indemnizar y mantener indemne a ARCANUM y su desarrollador por reclamaciones derivadas de:
- Tu uso de la App
- Violación de estos Términos
- Violación de derechos de terceros

---

## 9. Disponibilidad del servicio

- Nos esforzamos por mantener alta disponibilidad, pero no garantizamos 100% uptime
- Podemos realizar maintenance programado con aviso previo
- Nos reservamos el derecho de modificar o descontinuar funciones con aviso razonable

---

## 10. Cambios en los Términos

- Nos reservamos el derecho de actualizar estos Términos
- Te notificaremos de cambios significativos a través de la App
- El uso continuado después de los cambios constituye aceptación
- Revisa periódicamente esta página

---

## 11. Ley aplicable y jurisdicción

Estos Términos se rigen por las leyes de la República de Colombia. Cualquier disputa será sometida a la jurisdicción de los tribunales competentes en Colombia.

---

## 12. Disposiciones generales

- Si alguna cláusula es declarada inválida, las demás continúan vigentes
- La renuncia a algún derecho no constituye renuncia a otros
- Estos Términos constituyen el acuerdo completo entre tú y ARCANUM

---

## 13. Contacto

Para preguntas sobre estos Términos de Servicio:

- **Correo electrónico:** soporte@arcanum-app.com
- **Desarrollador:** Samuel Escobar
- **País:** Colombia

---

*Gracias por usar ARCANUM.*
